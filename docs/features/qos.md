...
**Matching is by specificity, not by list order.** A classifier encodes "any" as a zero in the match tuple, and the data plane finds it by rebuilding the key from the packet with the open fields zeroed out. It walks from the most specific shape to the least: exact 5-tuple, then wildcard source port, then both ports, then a port pair on any host, then destination port alone, and then source port alone. The whole host and port ladder runs first at the packet's own DSCP, and only then a second time with the DSCP left open, because an operator who wrote the DSCP down meant it to take precedence. That is the ladder run twice rather than a DSCP test inside each step, so a rule naming a destination port and a DSCP wins over a rule naming an exact 5-tuple and no DSCP. After both passes come the rules that name neither a host nor a port: a DSCP-only rule, then a protocol-wide rule, then the wildcard (`match: {}`).
