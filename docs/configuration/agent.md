...
This block governs both ports. The enterprise agent reads the same three fields and applies the same two tiers to its own API, so a deployment running both tunes one thing rather than two.
